...
var Iconify = (function () {
	...
	var storage_2 = storage_1.listIcons;
	var storage_3 = storage_1.getIcon;
	var storage_4 = storage_1.iconExists;
	var storage_5 = storage_1.addIcon;
	var storage_6 = storage_1.addIconSet;
	var storage_7 = storage_1.getStorage;
	var storage_8 = storage_1.newStorage;
	 ...
	 */
	exports.storeCache = function (provider, data) {
	    if (!loaded) {
	        exports.loadCache();
	    }
	    function store(key) {
	...
	var storage_2$1 = storage.storeCache;
	var storage_3$1 = storage.loadCache;
	var storage_4$1 = storage.mock;
	var storage_5$1 = storage.emptyList;
	var storage_6$1 = storage.count;
	var storage_7$1 = storage.config;
	...
	var callbacks_1 = callbacks.storeCallback;
	var callbacks_2 = callbacks.updateCallbacks;
	var callbacks_3 = callbacks.callbacks;
	...
	var modules_1$1 = modules$1.getAPIModule;
	var modules_2 = modules$1.setAPIModule;
	 ...
	 */
	exports.getAPIConfig = function (provider) { return configStorage[provider]; };

	});

	unwrapExports(config$1);
	var config_1$1 = config$1.getAPIConfig;
	var config_2 = config$1.setAPIConfig;
	...
	unwrapExports(list);
	var list_1 = list.getProviders;
	var list_2 = list.listToIcons;
	...
	unwrapExports(api);
	var api_1 = api.API;
	var api_2 = api.getRedundancyCache;
	 ...
	 * Return API module
	 */
	exports.getAPIModule = function (getAPIConfig) {
	    /**
	...
	unwrapExports(jsonp);
	var jsonp_1 = jsonp.getAPIModule;
	...
	var fetch_2 = fetch_1.getAPIModule;
	...
	function scanDOM(node, addTempNode) {
	    ...
	    (node ? [node] : listRootNodes()).forEach(function (node) {
	        ...
	        findPlaceholders(root).forEach(function (item) {
	            ...
	            // Check icon
	            var storage = storage_7(provider, prefix);
	            if (storage.icons[name] !== void 0) {
	                ...
	                // Render icon
	                renderIcon(item, customisations, storage_3(storage, name));
	                return;
	            }
	...
	function getIconData(name) {
	    ...
	    return icon
	        ? storage_3(storage_7(icon.provider, icon.prefix), icon.name)
	        : null;
	}
	/**
	 * Get SVG data
	...
	function addCollection(data, provider) {
	    ...
	    var storage = storage_7(provider, data.prefix);
	    return !!storage_6(storage, data);
	}
	/**
	 * Global variable
	 */
	var IconifyCommon = {
	    // Version
	    getVersion: function () { return '2.0.0-beta.6'; },
	    // Check if icon exists
	    ...
	    addIcon: function (name, data) {
	        ...
	        var storage = storage_7(icon.provider, icon.prefix);
	        return storage_5(storage, icon.name, data);
	    },
	    // Add icon set
	...
	function toggleCache(storage, value) {
	    switch (storage) {
	        case 'local':
	        case 'session':
	            storage_7$1[storage] = value;
	            break;
	        case 'all':
	            for (var key in storage_7$1) {
	                storage_7$1[key] = value;
	            }
	            break;
	    }
	}
	...
	var Iconify = {
	    ...
	    _internal: {
	        ...
	        getAPI: api_2,
	        // Get API config
	        getAPIConfig: config_1$1,
	        // Get API module
	        setAPIModule: modules_2,
	    },
	};
	...
	try {
	    ...
	    getAPIModule = jsonp_1;
	}
	modules_2('', getAPIModule(config_1$1));
	if (typeof document !== 'undefined' && typeof window !== 'undefined') {
	    // Set cache and load existing cache
	    modules_1.cache = storage_2$1;
	    storage_3$1();
