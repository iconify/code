...
function getIconData(name) {
  const icon = typeof name === "string" ? stringToIcon$1(name, true, simpleNames) : name;
  return icon ? getIconFromStorage(getStorage(icon.provider, icon.prefix), icon.name) : null;
}
function addIcon(name, data) {
  const icon = stringToIcon$1(name, true, simpleNames);
  if (!icon) {
    return false;
  }
...
function listToIcons(list, validate = true, simpleNames = false) {
  const result = [];
  list.forEach((item) => {
    const icon = typeof item === "string" ? stringToIcon$1(item, false, simpleNames) : item;
...
export { _api, addAPIProvider, addCollection, addIcon, buildIcon, calculateSize, Iconify as default, disableCache, enableCache, getIcon, getVersion, iconExists, listIcons, loadIcons, observe, pauseObserver, renderHTML, renderIcon, renderSVG, replaceIDs, resumeObserver, scan, stopObserving };
