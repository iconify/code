...
var Iconify = (function (exports) {
  ...
  // src/icon/name.ts
  var stringToIcon$1 = function (value, validate, allowSimpleName, provider) {
    ...
    if (dashSeparated.length > 1) {
      ...
      return validate && !validateIcon$1(result$1) ? null : result$1;
    }
    if (allowSimpleName && provider === "") {
      ...
      return validate && !validateIcon$1(result$2, allowSimpleName) ? null : result$2;
    }
    return null;
  };
  var validateIcon$1 = function (icon, allowSimpleName) {
    if (!icon) {
      return false;
    }
  ...
  function getIconData(name) {
    var icon = typeof name === "string" ? stringToIcon$1(name, true, simpleNames) : name;
    return icon ? getIconFromStorage(getStorage(icon.provider, icon.prefix), icon.name) : null;
  }
  function addIcon(name, data) {
    var icon = stringToIcon$1(name, true, simpleNames);
    if (!icon) {
      return false;
    }
  ...
  function calculateSize$1(size, ratio, precision) {
    ...
    var oldParts = size.split(unitsSplit$1);
    if (oldParts === null || !oldParts.length) {
      return size;
    }
    var newParts = [];
    var code = oldParts.shift();
    var isNumber = unitsTest$1.test(code);
    while (true) {
      if (isNumber) {
  ...
  function replaceIDs(body, prefix) {
    ...
    ids.forEach(function (id) {
      ...
      parts.forEach(function (part) {
        ...
        var lastChar = lastPart.slice(lastPart.length - 1);
        var nextChar = part.slice(0, 1);
        function test() {
          if (lastChar === "#") {
            return isQuote(nextChar) || nextChar === ")";
          }
          var isAnimationDot = nextChar === "." && !!part.slice(1, 2).match(/\w/);
  ...
  function listToIcons(list, validate, simpleNames) {
    ...
    list.forEach(function (item) {
      var icon = typeof item === "string" ? stringToIcon$1(item, false, simpleNames) : item;
      if (!validate || validateIcon$1(icon, simpleNames)) {
        result.push({
