...
function getRenderMode(body, mode) {
    ...
    if (mode !== 'style' && body.indexOf('<a') === -1) {
        // Render <svg>
        return 'svg';
    }
...
function buildIcon$1(icon, customisations) {
...
function exportFunctions() {
    ...
    return {
        ...
        shareStorage: shareStorage$1,
        addIcon: addIcon$1,
        addCollection: addCollection$1,
        calculateSize: calculateSize$1,
        buildIcon: buildIcon$1,
...
const viewBoxProps = ['left', 'top', 'width', 'height'];
/**
 * Render icon
 */
        ...
        if (parts.length === 4) {
            for (let i = 0; i < 4; i++) {
                ...
                if (isNaN(num)) {
                    ...
                    customisedViewBox[viewBoxProps[i]] = num;
                }
            }
            if (!failed) {
 ...
 * Export functions
 */
const { enableCache, disableCache, iconExists, getIcon, listIcons, shareStorage, addIcon, addCollection, calculateSize, replaceIDs, buildIcon, loadIcons, loadIcon, addAPIProvider, _api, } = IconifyIconComponent;

export { IconifyIconComponent, _api, addAPIProvider, addCollection, addIcon, buildIcon, calculateSize, disableCache, enableCache, getIcon, iconExists, listIcons, loadIcon, loadIcons, replaceIDs, shareStorage };
